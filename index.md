...
The task of voice conversion(VC) system is to change the voice identity from one speaker into another while keeping the linguistic content. Any-to-any voice conversion had gained increasing popularity in many applications. In the paper, we presented FlowCPCVC, a zero-shot voice conversion(VC) system, which is a light-weight end-to-end system and can achieve state-of-art quality. Our model base on VAE, it combined speaker encoder and contend extractor with a fancy method for training. The speaker encoder aim to extract speaker's timbre and contend extractor, which construct with Contrastive predictive coding(CPC) learning network, help to extract linguistic content to guide the flow module discard the timbre and keeping the linguistic content. Using the  speaker encoder module, our framework can extract the timbre embedding vector from speech to assist to finish the zero-shot VC task. Although our framework force on any-to-any task, it can also extend to any-to-many task for better robustness. Experimental results show that the FlowCPCVC system, compared with other zero-shot systems, can achieve state-of-art quality for voice conversion.
